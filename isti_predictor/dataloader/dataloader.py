...
class VideoLoader(preprocess_face, non_linearity):
        ...
        def __call__(self, vid_fname):
                ...
                if 'test' in self.vid_path[0]:
                        ...
                        new_video = []
                        for i in range(video_mat.shape[2]):
                                slice_2d = video_mat[:, :, i]
                                # print(slice_2d.shape)
                                #resized = np.reshape(slice_2d, (640, 480))
                                resized = cv2.resize(slice_2d, (4 * 220 // 3, 220))
                                new_video.append(np.flip(np.pad(resized[:, :240], [(640-220, 0), (0,0)], mode='constant', constant_values=np.min(resized)), axis=0))
                                # plt.show()
                        video_mat = np.array(new_video)
                        # breakpoint()
